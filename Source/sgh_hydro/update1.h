...
/* predictor step */
  ...
  for (auto v : verts ) {
    ...
    //save original nodal position for remap
    for (j=0;j<3;j++)
      node[v].x0[j]=mesh.points[v].px[j];
      
    if (vflag[v]) {

      for (j=0;j<3;j++) {
	mesh.points[v].px[j]=mesh.points[v].px[j]+node[v].dx[j];
	node[v].un[j]=node[v].un[j]+node[v].du[j];
      }
